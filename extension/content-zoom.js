// Zoom Web Client Content Script for Caption Scraping
console.log("Synapse Scraper: Zoom Web Client content script loaded.");

const SELECTORS = {
    container: '.cc-window, .closed-caption-container, .caption-box',
    speaker: '.cc-speaker, .caption-speaker, .cc-window__speaker',
    text: '.cc-text, .caption-text, .cc-window__text'
};
...
function startObserving(containerElement) {
    ...
    console.log("Synapse Scraper: Activating robust observation on Zoom captions container.");
    chrome.runtime.sendMessage({ action: "captionsStatus", active: true, platform: "Zoom Web" });
...
function handleCaptionMutations() {
    ...
    // Direct children of the caption container serve as individual turns/sentences
            ...
            if (textContainer) {
                ...
                for (const child of textContainer.childNodes) {
                    if (child.nodeType === 3) { // TEXT_NODE
                        ...
                        // Filter out speaker prefix in text
                        if (val && !val.endsWith(':')) {
...
function stopObserving() {
    if (observer) {
        observer.disconnect();
        observer = null;
        console.log("Synapse Scraper: Zoom observation stopped.");
    }
    chrome.runtime.sendMessage({ action: "captionsStatus", active: false, platform: "Zoom Web" });
}
